...
위의 예제처럼 객체가 한두 개라면 넘어 갈 수 있겠지만 만약 수십 개의 객체를 생성해야 한다면 문제가 크다.
...
자바와 같은 클래스 기반 객체지향 언어의 생성자와는 다르게 그 형식이 정해져 있는 것이 아니라 일반 함수와 동일한 방법으로 생성자 함수를 정의하고
...
따라서 함수 객체는 일반 객체가 가지고 있는 내부 슬롯과 메서드는 물론,
...
그렇지 않은 함수 객체를 non-constructor라고 부른다.
...
함수가 어디에 할당되어 있는지에 따라 판단하는 것이 아니라 함수 정의 방식에 따라 구분한다.
...
`Circle`함수를 new 연산자와 함께 호출하면 함수 내부의 `this`는 `Circle` 생성자 함수가 생성할 인스턴스를 가리킨다.
하지만 일반 함수로 호출하면 함수 내부의 `this`는 전역 객체 window를 가리킨다.

### 2.7. new.target

생성자 함수가 new 연산자 없이 호출되는 것을 방지하기 위해 파스칼 케이스 컨벤션을 사용한다 하더라도 실수는 언제나 발생할 수 있다.
이러한 위험성을 회피하기 위해 ES6에서 `new.target`을 지원한다.
...
하지만 String, Number, Boolean 생성자 함수는 new 연산자와 함께 호출했을 때 해당 객체를 생성하여 반환하지만
new 연산자 없이 호출하면 해당 값을 반환한다. 이를 통해 데이터 타입을 변환하기도 한다.
